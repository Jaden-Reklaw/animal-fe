...
export default function CreateAnimals({ dataService }: CreateAnimalsProps) {
    ...
    const handleSubmit = async (event: SyntheticEvent) => {
        ...
        if (name && location) {
            const id = await dataService.createAnimals(name, location, photo)
            setActionResult(`Created animal with id ${id}`);
            setName('');
            setLocation('');
        } else {
